...
class LRFigures:
    ...
    def plot(self, metrics:List[str], number_of_answers:str, values:List[float]) -> None:

        for index, metric in enumerate(metrics):
            if metric in self.metrics:
                self.metrics[metric]['labels'].append(number_of_answers)
                self.metrics[metric]['values'].append(values[index])
            else:
                self.metrics[metric] = {
    ...
    def save(self, config:Configuration):
        ...
        for metric_name, metric in self.metrics.items():
            ...
            plt.plot(metric["labels"], metric["values"], 'ro')

            filename = f"lexical_richness_{self.data_source}_{metric_name}.pdf"
            log.debug(f"Save Diagram: {filename}")
...
def _calculate_and_save_lexical_richness(path, answers_per_question:Dict[str, Answer], data_source:str, number_of_answers:Union[int, None], diagrams:LRFigures) -> None:
    ...
    with open(path, mode, newline='') as tsvfile:
        ...
        for question, answers_for_question in answers_per_question.items():
            ...
            diagrams.plot(labels, len(answers), total_lr)
...
if __name__ == "__main__":
    ...
    for size in [None, 100, 200, 400, 800, 1600, 3200]:
